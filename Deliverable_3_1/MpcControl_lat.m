classdef MpcControl_lat < MpcControlBase
    ...
    methods
        ...
        function ctrl_opti = setup_controller(mpc)
            ...
            [Xf, Qf] = terminal_invariant_set(mpc);
            [Ff, ff] = double(Xf); % half space representation for Xf
            Q = diag([1, 1]); % y, theta
            R = 1; % steering angle
        ...
        function [O, Qf] = terminal_invariant_set(mpc)
            A = mpc.A;
            B = mpc.B;
            ...
            Hu = [1; -1]; 
            hu = [deg2rad(30) - us; deg2rad(30) + us];

            HH = [H; Hu * K]; 
            hh = [h; hu]; % State and input constraints
